...
## 개요
### 🚗니차내차
> <font size="3">중고차 시장 데이터를 수집·분석하여 차량의 평균 시세, 가격 범위, 자차 견적을 제공하고, </br>
사용자가 합리적인 가격으로 거래할 수 있도록 돕는 데이터 기반 플랫폼</br></br>
> <필요성></br>
> 중고차의 특성상 구매자는 광범위한 정보를 수집하는데 제한이 있습니다.</br>
> 이를 한눈에 볼 수 있도록 데이터를 제공하는 것은 구매자의 정보비대칭을 해소하고 과도한 지출을 방지할 수 있습니다.</br></br>
> <목표> </br>1. 정보 비대칭 해소</br>
> 2. 과도한 지출 방지</br>
> 3. 적정 가격 제공</br>
> 4. 시장 트렌드 제공</br>
> 5. 허위 매물 필터링
...
### 요구사항 명세서
<중고차 시세 조회 웹 서비스 요구사항 명세서>

1. 개요</br>
1.1 목적</br>
사용자가 원하는 조건의 중고차를 검색하고, 시세를 확인하며, 관심 차량을 저장(찜)하고, 자신의 차량 시세를 비교할 수 있는 웹 기반 서비스 제공
...
2. 사용자 정의
- 일반 사용자: 차량 시세 조회 및 비교, 찜 기능 사용

3. 기능 요구사항

3.1 중고차 시세 조회 (List Page)
- 제조사, 모델, 연료, 연식, 주행거리 조건 검색
- 차량 리스트 카드 형태 출력
- 카드 정보: 이미지, 제조사/모델명, 연식, 주행거리, 연료, 가격
- 페이지네이션: 이전/다음 버튼, 첫/마지막 페이지 비활성화

3.2 차량 카드 UI
- 이미지, 차량 정보, 가격, 찜 버튼 포함
- hover 효과, 클릭 인터랙션
- 카드 높이 균일, 간격 유지, 반응형 구성
...
- 성능: 1초 이내 응답
- 사용성: 직관적 UI
- 보안: DB 정보 보호, SQL Injection 방지
- 유지보수: 모듈 구조 설계

5. 시스템 구조
Streamlit UI → Service Layer → Repository Layer → Database
